...
4. **How it works:** the 4-step flow — photograph the printer → AI-generated config (with generic-config fallback) → one-time entry at the printer → scans land in your inbox automatically, plus the day-to-day usage line (press scan, pick the address, done).
